...
path_config() {
	echo
	while :; do
		echo -e "Please input the ${magenta} diffluence path $none , for example /233blog , only 233blog need to input"
		...
		*[/$]*)
			echo
			echo -e " This script is a shit. So the diffluence path can't include $red / $noneor$red $ $none.... "
			echo
			error
			;;
		*)
...
proxy_site_config() {
	echo
	while :; do
		echo -e "Please input ${magenta} a normal $none ${cyan}URL$none to act as  ${cyan}website disguise$none , for example https://liyafly.com"
		echo -e "For example...Your current domain name is  $green $domain$none , the disguising URL will be https://liyafly.com"
		echo -e "And when you open your domain name ...the content from https://liyafly.com will be shown."
		...
		*[#$]*)
			echo
			echo -e " This scrip is a shit..So the disguiseing URL can't include $red # $noneor$red $ $none.... "
			echo
			error
			;;
		*)
...
shadowsocks_password_config() {
	...
	while :; do
		...
		*[/$]*)
			echo
			echo -e " This script is a shit..So password can't include $red / $noneor$red $ $none.... "
			echo
			error
			;;
		*)
...
uninstall() {
	...
	if [[ -f /usr/bin/v2ray/v2ray && -f /etc/v2ray/config.json ]] && [[ -f $backup && -d /etc/v2ray/233boy/v2ray ]]; then
		...
		Note...Can only remove (233v2.com)  V2Ray script
		" && exit 1
	fi
...
while :; do
	echo
	echo "........... V2Ray installation & management script by 233v2.com .........."
	echo
	echo "Help: https://233v2.com/post/1/"
	echo
	echo "Guide: https://233v2.com/post/2/"
	echo
	echo " 1. Install"
	echo
	echo " 2. Remove"
