while :; do
	echo
	read -p "$(echo -e "wanna uninstall ${yellow}V2Ray$none [${magenta}Y/N$none]:")" uninstall_v2ray_ask
	if [[ -z $uninstall_v2ray_ask ]]; then
		...
		N | n)
			echo
			echo -e "$red cancel to uninstall...$none"
			echo
			break
			;;
		*)
...
if [[ $caddy && $is_uninstall_v2ray ]] && [[ -f /usr/local/bin/caddy && -f /etc/caddy/Caddyfile ]]; then
	while :; do
		echo
		read -p "$(echo -e "wanna uninstall ${yellow}Caddy$none [${magenta}Y/N$none]:")" uninstall_caddy_ask
...
if [[ $is_uninstall_v2ray && $is_uninstall_caddy ]]; then
	...
	# clear
	echo
	echo -e "$green V2Ray complete to uninstall....$none"
	echo
	...
	# clear
	echo
	echo -e "$green V2Ray uninstallation complete ....$none"
	echo
fi
